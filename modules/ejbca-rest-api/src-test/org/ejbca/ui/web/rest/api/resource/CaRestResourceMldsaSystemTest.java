...
public class CaRestResourceMldsaSystemTest extends RestResourceSystemTestBase {

    private static final Logger log = Logger.getLogger(CaRestResourceMldsaSystemTest.class);
    
    private static final String TEST_ISSUER_MLDSA44 = "CN=CaRestResourceSystemTestMLSDA44-" + new Random().nextInt();
    ...
    public void setUp() throws Exception {
        ...
        cainfo.setDefaultCRLDistPoint("http://localhost:8080/ejbca/publicweb/webdist/certdist" + "?cmd=crl&issuer=CN%3D" + TEST_ISSUER_MLDSA44 + "&partition=*");
        caAdminSession.createCA(INTERNAL_ADMIN_TOKEN, cainfo);
    }

    @After
    public void tearDown() throws Exception {        
        CaRestResourceSystemTest.removeCa((X509CAInfo) caSession.getCAInfo(INTERNAL_ADMIN_TOKEN, TEST_ISSUER_MLDSA44.hashCode()));
        Files.deleteIfExists(Paths.get(CRL_FILENAME));
    }
    
    @Test
    public void testCreateCrlWithoutDeltaCrlMLDSA44() throws Exception {
        // without delta
        String responseBody = CaRestResourceSystemTest.createCrl("/v1/ca/" + encodeUrl(TEST_ISSUER_MLDSA44) + "/createcrl?deltacrl=false");
        ...
        // with delta
        responseBody = CaRestResourceSystemTest.createCrl("/v1/ca/" + encodeUrl(TEST_ISSUER_MLDSA44) + "/createcrl?deltacrl=true");
        CaRestResourceSystemTest.assertTrue(responseBody, ("\"all_success\":true"));
