...
public class MSEnrollmentServlet extends HttpServlet {
    ...
    void doSoapRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            if (debug2) {
                System.out.println("Request AuthType: [" + request.getAuthType() + "]");
                System.out.println("Request RemoteUser: [" + request.getRemoteUser() + "]");
                System.out.println(
                        "Request UserPrincipal: [" + request.getUserPrincipal().toString() + "], [" + request.getUserPrincipal().getName() + "]");
    ...
    void handlePkcs10request(String contents, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        PrintWriter out = response.getWriter();
        ...
        try {
            ...
            CertificateResponse certResponse = ws.certificateRequest(user1, pkcs10request, CertificateHelper.CERT_REQ_TYPE_PKCS10, null,
                    CertificateHelper.RESPONSETYPE_PKCS7WITHCHAIN);
            ...
            try {
             ...
             //   String activityId = findTagValue("ActivityId", contents);
                ...
                String strCreated = df1.format(certResponse.getCertificate().getNotBefore()) + "T"
                        + df2.format(certResponse.getCertificate().getNotBefore()) + "Z";
                sbOut.append("<u:Created>").append(strCreated).append("</u:Created>");
                sbOut.append("<u:Expires>").append(df1.format(certResponse.getCertificate().getNotAfter())).append("T")
                        .append(df2.format(certResponse.getCertificate().getNotAfter())).append("Z</u:Expires>");
                ...
                sbOut.append(
                        "<DispositionMessage xml:lang=\"en-US\" xmlns=\"http://schemas.microsoft.com/windows/pki/2009/01/enrollment\">Issued</DispositionMessage>");
