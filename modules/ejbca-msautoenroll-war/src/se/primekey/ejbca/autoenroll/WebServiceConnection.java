	...
	java.security.KeyStore loadTrustKeyStore() throws Exception
	{
		...
		try
		{
			ks = java.security.KeyStore.getInstance(java.security.KeyStore
					.getDefaultType());
			// get user password and file input stream
			char[] password = properties.getTrustedKeyStorePassword()
					.toCharArray();
		...
		try
		{
			...
			if (bUseSystemProperties)
			{
				...
				System.setProperty("javax.net.ssl.trustStorePassword",
						properties.getTrustedKeyStorePassword());
...
//			exc.printStackTrace();
	...
	void setNewDefaultSSLSocketFactory()
			throws Exception
	{
		ApplicationProperties properties = getProps();
		...
		try
		{
			KeyManagerFactory kmf = KeyManagerFactory
					.getInstance("SunX509");
			kmf.init(loadKeyStore(), properties.getKeyStorePassword()
					.toCharArray());
...
//			EjbcaWSService service = new EjbcaWSService(new URL(urlstr), qname);
		...
		catch (Exception exc)
		{
			exc.printStackTrace();
			System.err
					.println("*** Could not connect to non-authenticated web service call getEjbcaVersion()");
