...
public class BatchMakeP12 extends BaseCommand {
	...
	public String getSubCommand() { return "batch"; }
	public String getDescription() { return "Generate keys and certificates for all users with status NEW"; }
    ...
    private Certificate[] getCACertChain(int caid) throws Exception {
        ...
        Certificate[] chain = (Certificate[]) getSignSession().getCertificateChain(getAdmin(), caid).toArray(new Certificate[0]);
        getLogger().trace("<getCACertChain()");
        return chain;
    }
     ...
     */
    private void storeKeyStore(KeyStore ks, String username, String kspassword, boolean createJKS, boolean createPEM)
    ...
    private void createUser(String username, String password, int caid, KeyPair rsaKeys, boolean createJKS, boolean createPEM, boolean savekeys, X509Certificate orgCert)
        ...
        ISignSessionRemote signSession = getSignSession();
        
        X509Certificate cert = null;
        
        if(orgCert != null){
          cert = orgCert;
          boolean finishUser = getCAAdminSession().getCAInfo(getAdmin(),caid).getFinishUser();
          if(finishUser){
        	  getAuthenticationSession().finishUser(getAdmin(), username, password);
          }
        ...
        if (getUseKeyRecovery() && savekeys ) {
            ...
            getKeyRecoverySession().addKeyRecoveryData(getAdmin(), cert, username, rsaKeys);
        }
    ...
    private void processUser(UserDataVO data, boolean createJKS, boolean createPEM, boolean keyrecoverflag) throws Exception {
        ...
        if (getUseKeyRecovery() && keyrecoverflag) {
        	boolean reusecertificate = getRaAdminSession().getEndEntityProfile(getAdmin(), data.getEndEntityProfileId()).getReUseKeyRevoceredCertificate();
            // Recover Keys
            IKeyRecoverySessionRemote keyrecoverysession = getKeyRecoverySession();
    ...
    private boolean doCreate(UserDataVO data, int status) throws Exception {
        ...
        if (createP12 || createPEM || createJKS) {
        	...
        	IUserAdminSessionRemote userAdminSession = getUserAdminSession();
    ...
    public void createAllWithStatus(int status) throws Exception {
        ...
        CertTools.installBCProviderIfNotAvailable();	// If this is invoked directly
        ArrayList result;
        IUserAdminSessionRemote userAdminSession = getUserAdminSession();
    ...
    public void createUser(String username) throws Exception {
        ...
        if ((data != null) && (data.getPassword() != null) && (data.getPassword().length() > 0)) {
            if ((status == UserDataConstants.STATUS_NEW) ||
                    ((status == UserDataConstants.STATUS_KEYRECOVERY) && getUseKeyRecovery())) {
                try {
    ...
    /**
     * Return environment variable EJBCA_HOME or an empty string if the variable isn't set.
