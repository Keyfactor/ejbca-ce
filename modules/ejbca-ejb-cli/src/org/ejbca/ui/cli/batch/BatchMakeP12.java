...
public class BatchMakeP12 extends BaseCommand {
   ...
   // private final AuthenticationSubject subject = getAuthenticationSubject("BatchMakeP12", "BatchMakeP12");
    ...
    private Certificate[] getCACertChain(String cliUserName, String cliPassword, int caid) throws AuthorizationDeniedException {
        ...
        Certificate[] chain = (Certificate[]) ejb.getSignSession().getCertificateChain(getAdmin(cliUserName, cliPassword), caid).toArray(new Certificate[0]);
        getLogger().trace("<getCACertChain()");
        return chain;
    }
    ...
    private void createUser(String cliUserName, String cliPassword, String username, String password, int caid, KeyPair rsaKeys, boolean createJKS, boolean createPEM, boolean savekeys,
        ...
        if (orgCert != null) {
            cert = orgCert;
            boolean finishUser = ejb.getCaSession().getCAInfo(getAdmin(cliUserName, cliPassword), caid).getFinishUser();
            if (finishUser) {
            	EndEntityInformation userdata = ejb.getEndEntityAccessSession().findUser(getAdmin(cliUserName, cliPassword), username);
                ejb.getEndEntityAuthenticationSession().finishUser(userdata);
            }
            ...
            cert = (X509Certificate) ejb.getSignSession().createCertificate(getAdmin(cliUserName, cliPassword), username, password, selfcert);
        }
        ...
        if (getUseKeyRecovery(cliUserName, cliPassword) && savekeys) {
            ...
            ejb.getKeyRecoverySession().addKeyRecoveryData(getAdmin(cliUserName, cliPassword), cert, username, rsaKeys);
        }
    ...
    private void processUser(String cliUserName, String cliPassword, EndEntityInformation data, boolean createJKS, boolean createPEM, boolean keyrecoverflag) throws Exception {
        ...
        if (getUseKeyRecovery(cliUserName, cliPassword) && keyrecoverflag) {
            ...
            if (reusecertificate) {
                ejb.getKeyRecoverySession().unmarkUser(getAdmin(cliUserName, cliPassword), data.getUsername());
            }
    ...
    private boolean doCreate(String cliUserName, String cliPassword, EndEntityInformation data, int status) throws Exception {
        ...
        if (createP12 || createPEM || createJKS) {
            ...
            processUser(cliUserName, cliPassword, data, createJKS, createPEM, (status == UserDataConstants.STATUS_KEYRECOVERY));
            ...
            EndEntityInformation vo = ejb.getEndEntityAccessSession().findUser(getAdmin(cliUserName, cliPassword), data.getUsername());
            if ((vo.getStatus() == UserDataConstants.STATUS_NEW) || (vo.getStatus() == UserDataConstants.STATUS_FAILED)
                ...
                ejb.getUserAdminSession().setClearTextPassword(getAdmin(cliUserName, cliPassword), data.getUsername(), data.getPassword());
            } else {
                // Delete clear text password, if we are not letting status be
                // the same as originally
                ejb.getUserAdminSession().setClearTextPassword(getAdmin(cliUserName, cliPassword), data.getUsername(), null);
            }
            ret = true;
    ...
    public void createAllNew(String cliUserName, String cliPassword) throws Exception {
        ...
        getLogger().info(iMsg);
        createAllWithStatus(cliUserName, cliPassword, UserDataConstants.STATUS_NEW);
        getLogger().trace("<createAllNew");
    }
    ...
    public void createAllFailed(String cliUserName, String cliPassword) throws Exception {
        ...
        getLogger().info(iMsg);
        createAllWithStatus(cliUserName, cliPassword, UserDataConstants.STATUS_FAILED);
        getLogger().trace("<createAllFailed");
    }
    ...
    public void createAllKeyRecover(String cliUserName, String cliPassword) throws Exception {
        if (getUseKeyRecovery(cliUserName, cliPassword)) {
            ...
            getLogger().info(iMsg);
            createAllWithStatus(cliUserName, cliPassword, UserDataConstants.STATUS_KEYRECOVERY);
            getLogger().trace("<createAllKeyRecover");
        }
    }
                ...
                for(EndEntityInformation data : result) {
                    if ((data.getPassword() != null) && (data.getPassword().length() > 0)) {
                        try {
                            ...
                            failcount++;
                            if (status == UserDataConstants.STATUS_KEYRECOVERY) {
                                ejb.getUserAdminSession().setUserStatus(getAdmin(cliUserName, cliPassword), data.getUsername(), UserDataConstants.STATUS_KEYRECOVERY);
                            } else {
                                ejb.getUserAdminSession().setUserStatus(getAdmin(cliUserName, cliPassword), data.getUsername(), UserDataConstants.STATUS_FAILED);
                            }
                        }
                    } else {
    ...
    public void createUser(String cliUserName, String cliPassword, String username) throws Exception {
        ...
        if ((data != null) && (data.getPassword() != null) && (data.getPassword().length() > 0)) {
            if ((status == UserDataConstants.STATUS_NEW) || ((status == UserDataConstants.STATUS_KEYRECOVERY) && getUseKeyRecovery(cliUserName, cliPassword))) {
                try {
