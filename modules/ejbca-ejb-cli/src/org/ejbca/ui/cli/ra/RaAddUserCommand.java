...
public class RaAddUserCommand extends BaseRaAdminCommand {
	...
	public String getSubCommand() { return "adduser"; }
	public String getDescription() { return "Adds a user"; }
    ...
    public void execute(String[] args) throws ErrorAdminCommandException {
        try {
            ...
            if(usehardtokens){  
              hardtokenissueraliases = (String[]) getHardTokenSession().getHardTokenIssuerAliases(getAdmin()).toArray(new String[0]);             

              authorizedhardtokenprofiles = getHardTokenSession().getAuthorizedHardTokenProfileIds(getAdmin());
              hardtokenprofileidtonamemap = getHardTokenSession().getHardTokenProfileIdToNameMap(getAdmin());
            }  
            ...
            if ( (args.length < 9) || (args.length > 12) ) {
                ...
                getLogger().info("SubjectAltName is of form \"rfc822Name=<email>, dNSName=<host name>, uri=<http://host.com/>," +
            ...
            int caid = 0;
            try{
              caid = getCAAdminSession().getCAInfo(getAdmin(), caname).getCAId();
            }catch(Exception e){               
            }
            
            if(args.length > 9){
              // Use certificate type, no end entity profile.
              certificatetypeid = getCertificateStoreSession().getCertificateProfileId(getAdmin(), args[9]);
              getLogger().info("Using certificate profile: "+args[9]+", with id: "+certificatetypeid);
            }

            if(args.length > 10){
              // Use certificate type and end entity profile.
              profileid = getRaAdminSession().getEndEntityProfileId(getAdmin(), args[10]);
            ...
            if(args.length == 12 && usehardtokens){
              ...
              hardtokenissuerid = getHardTokenSession().getHardTokenIssuerId(getAdmin(),args[11]);
              usehardtokenissuer = true;
              getLogger().info("Using hard token issuer: "+args[11]+", with id: "+hardtokenissuerid);
            }
            
            int tokenid =getTokenId(getAdmin(), tokenname, usehardtokens, getHardTokenSession());
            if (tokenid == 0) {
            ...
            if(!error){
              ...
              try{
                getUserAdminSession().addUser(getAdmin(), username, password, dn, subjectaltname, email, false, profileid, certificatetypeid,
