...
public class AddEndEntityCommand extends BaseRaCommand {
    ...
    
    {
        ...
        registerParameter(new Parameter(SUBJECT_ALT_NAME_KEY, "Subject Name", MandatoryMode.OPTIONAL, StandaloneMode.FORBID, ParameterMode.ARGUMENT,
                "SubjectAltName is of form \"rfc822Name=<email>, dNSName=<host name>, uri=<http://host.com/>,"
                        ...
                        + " krb5principal=<Krb5 principal name>, permanentIdentifier=<Permanent Identifier values>, hardwareModuleName=<OID/serial>,"
    ...
    public CommandResult execute(ParameterContainer parameters) {
        ...
        final String endEntityProfileName = parameters.get(EE_PROFILE_KEY);
        ...
        boolean useClearTextPassword = false;
        boolean autoGeneratedPassword = false;
        ...
        if (endEntityProfileName != null) {
            ...
            try {
                endEntityProfileId = EjbRemoteHelper.INSTANCE.getRemoteSession(EndEntityProfileSessionRemote.class)
                        .getEndEntityProfileId(endEntityProfileName);

                //In some cases the clear text password can be set to true by default.
                EndEntityProfile endEntityProfile =
                        EjbRemoteHelper.INSTANCE.getRemoteSession(EndEntityProfileSessionRemote.class).getEndEntityProfile(endEntityProfileName);
                useClearTextPassword = endEntityProfile.isClearTextPasswordUsed() && endEntityProfile.isClearTextPasswordDefault();
                autoGeneratedPassword = endEntityProfile.useAutoGeneratedPasswd();
                getLogger().info("Default clear password is true: " + endEntityProfile.isClearTextPasswordUsed() + "," + endEntityProfile.isClearTextPasswordDefault());
            } catch (EndEntityProfileNotFoundException e) {
                getLogger().error("ERROR: Could not find end entity profile in database.");
                error = true;
            }
            getLogger().info("Using entity profile: " + endEntityProfileName + ", with id: " + endEntityProfileId);
        }
        ...
        if (!error) {
            ...
            if (!autoGeneratedPassword) {
                ...
                getLogger().info("Password: <auto-generated>)");
            }
            ...
            try {
                ...
                EjbRemoteHelper.INSTANCE.getRemoteSession(EndEntityManagementSessionRemote.class).addUser(getAuthenticationToken(), endEntityInformation, useClearTextPassword);
                ...
                if (!useClearTextPassword) {
                    ...
                    getLogger().info("Clear text password was set.");
                }
