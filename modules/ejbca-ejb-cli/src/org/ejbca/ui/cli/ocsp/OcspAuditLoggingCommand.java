...
public class OcspAuditLoggingCommand extends EjbcaCliUserCommandBase {
...

    {
        registerParameter(new Parameter(ENABLE_KEY,
                "OCSP audit logging status",
                MandatoryMode.OPTIONAL,
                StandaloneMode.ALLOW,
                ParameterMode.ARGUMENT,
                "Enable or disable OCSP audit logging. Set to 'true' to enable OCSP audit logging, or 'false' to disable."));
        ...
        registerParameter((new Parameter(LOG_VALUES_KEY,
                ...
                StandaloneMode.FORBID,
                ParameterMode.ARGUMENT,
                "The audit log message. Variables may be used as specified in the documentation.")));
    ...
    public CommandResult execute(final ParameterContainer parameters) {
        try {
            ...
            if (parameters.containsKey(ENABLE_KEY)) {
                ocspConfiguration.setIsOcspAuditLoggingEnabled(Boolean.parseBoolean(parameters.get(ENABLE_KEY)));
            }
            if (parameters.containsKey(LOG_PATTERN_KEY)) {
                ocspConfiguration.setOcspAuditLogPattern(parameters.get(LOG_PATTERN_KEY));
            }
            if (parameters.containsKey(LOG_VALUES_KEY)) {
                ocspConfiguration.setOcspAuditLogValues(parameters.get(LOG_VALUES_KEY));
            }
    ...
    public String getCommandDescription() {
        return "Configure OCSP audit logging.";
    }

    @Override
    public String getFullHelpText() {
        return "Configure whether OCSP audit logging should be enabled and what the audit log messages should look like.";
    }
    ...
    private void print(final GlobalOcspConfiguration ocspConfiguration) {
        log.info("enabled: " + ocspConfiguration.getIsOcspAuditLoggingEnabled());
        log.info("log_pattern: " + ocspConfiguration.getOcspAuditLogPattern());
        log.info("log_values: " + ocspConfiguration.getOcspAuditLogValues());
