 ...
 * 
 */
public class PrevRole extends UpgradeableDataHashMap implements Named, Comparable<PrevRole> {
    ...
    /** Copy constructor */
    public PrevRole(final PrevRole role) {
    ...
    public PrevRole(final String nameSpace, final String roleName) {
        this.roleId = ROLE_ID_UNASSIGNED;
        setNameSpace(nameSpace);
        this.roleName = roleName;
    }

    public PrevRole(final String nameSpace, final String roleName, final HashMap<String, Boolean> accessRules) {
        this.roleId = ROLE_ID_UNASSIGNED;
        setNameSpace(nameSpace);
        this.roleName = roleName;
    ...
    public PrevRole(final String nameSpace, final String roleName, final List<String> resourcesAllowed, final List<String> resourcesDenied) {
        this.roleId = ROLE_ID_UNASSIGNED;
        setNameSpace(nameSpace);
        this.roleName = roleName;
        if (resourcesAllowed!=null) {
            for (final String resource : resourcesAllowed) {
                getAccessRules().put(AccessRulesHelper.normalizeResource(resource), PrevRole.STATE_ALLOW);
            }
        }
        if (resourcesDenied!=null) {
            for (final String resource : resourcesDenied) {
                getAccessRules().put(AccessRulesHelper.normalizeResource(resource), PrevRole.STATE_DENY);
            }
        }
    }

    /** Constructor used during load from database */
    public PrevRole(final int roleId, final String nameSpace, final String roleName, final LinkedHashMap<Object, Object> dataMap) {
        this.roleId = roleId;
        setNameSpace(nameSpace);
        this.roleName = roleName;
        loadData(dataMap);
    ...
    @Override
    public int compareTo(PrevRole role) {
