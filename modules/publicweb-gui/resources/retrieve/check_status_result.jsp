...
<%@ include file="header.jsp" %>
    ...
    <c:choose> 
        <c:when test="${issuer == null || issuer == ''}"> 
            ...
            <p>Please enter a valid issuer in the <a href="<c:url value="check_status.jsp"> <c:param name="hidemenu" value="${hidemenu}"/> </c:url>" >search form</a>!</p>
        </c:when>
        <c:when test="${serno == null || serno == ''}"> 
            <h1 class="title">No serial number</h1> 
            <p>Please enter a valid serial number in the <a href="<c:url value="check_status.jsp"> <c:param name="hidemenu" value="${hidemenu}"/> </c:url>" >search form</a>!</p>
        </c:when>
        <c:otherwise> 
