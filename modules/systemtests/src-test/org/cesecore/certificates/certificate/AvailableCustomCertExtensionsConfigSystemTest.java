...
public class AvailableCustomCertExtensionsConfigSystemTest {
    ...
    private final AuthenticationToken alwaysAllowToken = new TestAlwaysAllowLocalAuthenticationToken(new UsernamePrincipal(
            "AvailableCustomCertExtensionsConfigSystemTest"));
    ...
    /**
     * Serialization of custom classes is normally prohibited by EJBCA, and needs to be enabled by modifying cesecore.properties
     * 
    ...
    public void testAddingKnownExternalExtension() throws CertificateExtentionConfigurationException, AuthorizationDeniedException {
        ConfigurationBase originalGlobalConfig = globalConfigSession
                .getCachedConfiguration(AvailableCustomCertificateExtensionsConfiguration.CONFIGURATION_ID);
        try {
            cesecoreConfigurationProxySession.setConfigurationValue(CesecoreConfiguration.CUSTOM_CLASS_WHITELIST_KEY,
                    WidgetCustomExtension.class.getCanonicalName());
            AvailableCustomCertificateExtensionsConfiguration cceConfig = new AvailableCustomCertificateExtensionsConfiguration();
            Properties props = new Properties();
