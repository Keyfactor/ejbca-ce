 ...
 */
public class EstClientModeBasicSystemTest extends EstTestCase {

    private static final Logger log = Logger.getLogger(EstClientModeBasicSystemTest.class);
    ...
    public void testSimpleEnrollWithChallengePwd() throws Exception {
        ...
        try {
            ...
            PKCS10CertificationRequest p10 = generateCertReq(requestDN, "foo123", null, null, null, ec256, "SHA256WithECDSA");
            ...
            p10 = generateCertReq(requestDN, "bar123", null, null, null, ec256, "SHA256WithECDSA");
            reqmsg = Base64.encode(p10.getEncoded());
    ...
    public void testSimpleEnrollWithDnPwd() throws Exception {
        ...
        try {
            ...
            PKCS10CertificationRequest p10 = generateCertReq(requestDN, null, null, null, null, ec256, "SHA256WithECDSA"); // no challenge password
            byte[] reqmsg = Base64.encode(p10.getEncoded());
            sendEstRequest(alias, "simpleenroll", reqmsg, 400, "<html><head><title>Error</title></head><body>Invalid username, enrollment code, or end entity status</body></html>", null, null); 
            ...
            p10 = generateCertReq(requestDN, null, null, null, null, ec256, "SHA256WithECDSA"); // no challenge password
            reqmsg = Base64.encode(p10.getEncoded());
            sendEstRequest(alias, "simpleenroll", reqmsg, 400, "<html><head><title>Error</title></head><body>Invalid username, enrollment code, or end entity status</body></html>", null, null); 
    ...
    public void testSimpleEnrollWithChallengePwdAndUsernameInDn() throws Exception {
        ...
        try {
            ...
            PKCS10CertificationRequest p10 = generateCertReq(requestDN, "bar123", null, null, null, ec256, "SHA256WithECDSA");
            byte[] reqmsg = Base64.encode(p10.getEncoded());
            byte[] resp = sendEstRequest(alias, "simpleenroll", reqmsg, 200, null);
            assertNotNull("There must be response data to simpleenroll request", resp);
            final CMSSignedData respmsg = new CMSSignedData(Base64.decode(resp));
    ...
    public void testSimpleReenroll() throws Exception {
        ...
        try {
            ...
            resp = sendEstRequest(true, alias, "cacerts", null, 200, null, null, null);
            // If all was OK we should have gotten a base64 encoded certificates-only CMS message back. RFC7030 section 4.1.3
            assertNotNull("There must be response data to cacerts request", resp);
            final CMSSignedData msg = new CMSSignedData(Base64.decode(resp));
            final Store<X509CertificateHolder> cacertstore = msg.getCertificates();
            final Collection<X509CertificateHolder> cacerts = cacertstore.getMatches(null);
            assertEquals("EST test CA has a single CA certificate", 1, cacerts.size());
