...
public class NestedMessageContent extends BaseCmpMessage implements RequestMessage {
    ...
    public void setPKIMessageBytes(final PKIMessage msg) {
        try {
            ...
            log.error("Error getting encoded bytes from PKIMessage: ", GdprRedactionUtils.getRedactedException(e));
        }
        setMessage(msg);
    }
            ...
            for (final X509Certificate cert : racerts) {
                if (log.isDebugEnabled()) {
                    log.debug("Trying to verifying the NestedMessageContent using the RA certificate with subjectDN '"
                            + LogRedactionUtils.getSubjectDnLogSafe(cert.getSubjectX500Principal().toString()) + "'");
                }
                try {
                    cert.checkValidity();
                } catch (CertificateExpiredException | CertificateNotYetValidException e) {                  
                    if (log.isDebugEnabled()) {
                        log.debug("Certificate with subjectDN '" + LogRedactionUtils.getSubjectDnLogSafe(CertTools.getSubjectDN(cert)) + "' is not valid: " + e.getMessage());
                    }
                    continue;
                }
