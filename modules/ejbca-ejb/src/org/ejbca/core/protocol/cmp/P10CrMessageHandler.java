		...
		try {
			...
			if (cmpRequestMessage instanceof P10CrCertificationRequestMessage) {
                ...
                if(p10CrReq.getHeader().getProtectionAlg() != null) {
                    ...
                    p10CrReq.setPreferredDigestAlg(AlgorithmTools.getDigestFromSigAlgAndHandlePSS(p10CrReq.getHeader().getProtectionAlg(), null));
                    if (PKCSObjectIdentifiers.id_RSASSA_PSS.getId().equals(p10CrReq.getHeader().getProtectionAlg().getAlgorithm().getId())) {
                        p10CrReq.setPss(true);
                    }
