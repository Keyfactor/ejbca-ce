...
public class CertificateRequestSessionBean implements CertificateRequestSessionRemote, CertificateRequestSessionLocal {
    ...
    public byte[] processCertReq(AuthenticationToken admin, EndEntityInformation userdata, String req, int reqType, int responseType) 
        ...
        try {
            ...
            KeyPair keyPairToArchive = validateAndGetMsaeKeyPairToArchive(reqType, requestMessage);
    ...
    private KeyPair validateAndGetMsaeKeyPairToArchive(int reqType, RequestMessage requestMessage) throws CertificateCreateException {
        ...
        ((MsKeyArchivalRequestMessage) requestMessage).decryptPrivateKey("BC", getDummyPrivateKey());
        return ((MsKeyArchivalRequestMessage) requestMessage).getKeyPairToArchive();
    }
    
    private PrivateKey getDummyPrivateKey() {
        byte[] encodedPrivateKey = null; // TODO
        try {
            KeyFactory kf = KeyFactory.getInstance("RSA");
            return kf.generatePrivate(new PKCS8EncodedKeySpec(encodedPrivateKey));
        } catch (Exception e) {
