    ...
    public byte[] processCertReq(AuthenticationToken admin, EndEntityInformation userdata, String req, int reqType, int responseType) 
        ...
        try {
            ...
            if (reqType==CertificateConstants.CERT_REQ_TYPE_MS_KEY_ARCHIVAL) {
                // make sure EE profile allows key archival and key recovery is enabled etc
                // see KeyStoreCreateSessionBean
                // also encryption permission in the CSR, RSA key etc
                
                //TODO: need to decrypt and verify the private key here
                
                
                keyPairToArchive = ((MsKeyArchivalRequestMessage) requestMessage).getKeyPairToArchive();
            }
