 ...
 * 
 * Based on cesecore:
 *      AccessControlSession.java 338 2011-02-25 09:46:38Z tomas
 * 
 * @version $Id$
 * 
 */
public interface AccessControlSession {
    ...
    /** Helper method to clear the local access control rule cache. Normally the cache expires after configured time, 
     * but when modifying access rules on the local node we can force cache clearing so we don't have to wait.
     * Other nodes in a cluster will still wait until expire though. 
     * This should be called whenever "accessTreeUpdateSession.signalForAccessTreeUpdate()" is called,
     * it can't be called automatically from that method due to circular dependencies.
     */
