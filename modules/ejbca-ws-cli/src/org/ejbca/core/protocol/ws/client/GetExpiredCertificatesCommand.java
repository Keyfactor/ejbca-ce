...
public class GetExpiredCertificatesCommand extends EJBCAWSRABaseCommand implements IAdminCommand{
    ...
    public GetExpiredCertificatesCommand(String[] args) {
        super(args);
    }
    ...
    public void execute() throws IllegalAdminCommandException, ErrorAdminCommandException {
        ...
        try {   
            ...
            try{
                List<Certificate> result = getEjbcaRAWS().getCertificatesByExpirationTime(days, maxNumberOfResults);
                
                if(result==null || result.size() == 0){
                    getPrintStream().println("No certificate will expire within " + days + " days.");
                }else{
    ...
    protected void usage() {
        getPrintStream().println("Command used to find certificates that will expire within a specified number of days");
        getPrintStream().println("Usage : getexpiredcerts <numberOfDays> <maxNumberOfResults> <encoding (DER|PEM)> <outputpath>");
