...
public class EndEntityProfileUnitTest {
    ...
    public void testUserFulfillEndEntityProfileWithDnOverrideWithExcessDnFields() throws EndEntityProfileValidationException {
        ...
        profile.setCopy(DnComponents.COMMONNAME, 0, true);
        profile.setValue(EndEntityProfile.AVAILCAS, 0, Integer.toString(SecConst.ALLCAS));
        // Userdata has fields which are missing from the EEP
        EndEntityInformation userdata = new EndEntityInformation("foo", "CN=abc,O=testorg", 123, "", "", new EndEntityType(EndEntityTypes.ENDUSER),
                123, CertificateProfileConstants.CERTPROFILE_FIXED_ENDUSER,
                SecConst.TOKEN_SOFT_PEM, null);
        userdata.setPassword("foo123");
        // Should fulfill, because DN override is enabled and the EEP doesn't need to have all the DN fields
        profile.doesUserFulfillEndEntityProfile(userdata, certProfileWithDnOverride, false, null);
    }
    
    @Test
    public void testUserFulfillEndEntityProfileWithDnOverrideWithoutExcessDnFields() throws EndEntityProfileValidationException {
        CertificateProfile certProfileWithDnOverride = new CertificateProfile(CertificateProfileConstants.CERTPROFILE_FIXED_ENDUSER);
        // Set DN override as enabled
        certProfileWithDnOverride.setAllowDNOverride(true);
        EndEntityProfile profile = new EndEntityProfile();
        ...
        profile.setCopy(DnComponents.COMMONNAME, 0, true);
        profile.setValue(EndEntityProfile.AVAILCAS, 0, Integer.toString(SecConst.ALLCAS));
        // Userdata does not have fields which are missing from the EEP
        EndEntityInformation userdata = new EndEntityInformation("foo", "CN=abc", 123, "", "", new EndEntityType(EndEntityTypes.ENDUSER),
                123, CertificateProfileConstants.CERTPROFILE_FIXED_ENDUSER,
                SecConst.TOKEN_SOFT_PEM, null);
        userdata.setPassword("foo123");
        // Should fulfill
