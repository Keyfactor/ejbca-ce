...
public class LdapPublisherUnitTest {
    ...
    // This allows us to test LDAP operations without having a LDAP server.
    //
    ...
    // This TestLdapPublisher will use our special TestLDAPConnection.
    //
   ...
   String certTestEE2 = "";
    ...
    public void constructLdapDn_EmptyBaseDN() {
        // Aim: With an empty Base DN, the constructed DN will not have anything extra appended; especially not a comma "," at the end.
        final LdapPublisher publ = new LdapPublisher();
        publ.setBaseDN("");
    ...
    public void constructLdapDn_BlankBaseDN() {
        // Aim: A test variation of previous test, but here the Base DN is "blank" (by just having a space character).
        final LdapPublisher publ = new LdapPublisher();
    ...
    public void createIntermediateNodes_FullTreeCreated() {
        // Aim: Test the modified 'creatintermediateNodes()' can build a full LDAP tree, including additional DN components of DC, C, L, and ST
        //      Indirectly testing 'getOblectClassAttribute()' changes as well.
        ...
        // Set 4 LDAP read failures. The will emulate 'createIntermediateNodes()' recursively scanning up the top node "dc=root", 
        ...
        //
        // Check each node that was being added to the LDAP
        //
    ...
    public void createIntermediateNodes_NoParentNode() {
        // Aim: Testing an extreme case when there is no parent node, just the CN. This could happen if the Base DN is set to the whole tree.
    ...
    public void createIntermediateNodes_RecursionWillAlwaysEnd() {
        // Aim: Check that the recursion in the code cannot go rouge and loop forever.
        ...
        // Set huge number of LDAP read failures. The createIntermediateNodes() should only loop till parent node is empty ("")
        lc.numberOfReadErrorsBeforeResult = 100;
         
        //
        // Test creatIntermediateNodes() will always exit the recursion when parent node is blank.
        ...
        //
        // Check the first Add operation is for 'dc=Root'. A true LDAP server is unlikely to add a new root node, but
        // our special Test Publisher will allow it.
        //
        LDAPEntry le = lc.ldapAdds.get(0);
        ...
        // We can assume the other nodes were created based upon an earlier Test result.
    }


    @Test
    public void createIntermediateNodes_OtherLdapException() {
        // Aim: Checking that an unexpected LDAPExceptions will abort the recursion and throw an exception.
        ...
        //
        // Test creatIntermediateNodes() will throw exception
        
        try {
            publ.createIntermediateNodes( lc,  "CN=Name 1,L=Small Village,ST=Region ABC,C=SE,dc=EU,dc=Root");
            assertTrue( "Code should not have returned normally.", false);
        } catch (PublisherException e) {
    ...
    public void revokeCertificate_TwoCertsBeforeRevocation_OneCertRemains() {
       ...
       final TestLdapPublisher publ = new TestLdapPublisher();
        ...
        // Set some fake details, but we need something valid that will respond to the connection test (TCP ping).
