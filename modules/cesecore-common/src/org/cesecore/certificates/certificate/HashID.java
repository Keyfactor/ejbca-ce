...
public class HashID {
	...
	private static HashID getFromDN(X500Principal principal) {
		...
		if ( id.isOK ) {
			if ( log.isDebugEnabled() ) {
				log.debug("The DN '" + LogRedactionUtils.getSubjectDnLogSafe(principal.getName()) + "' is identified by the Hash string '" +
						id.b64 + "' when accessing the VA.");
			}
		} else {
			log.error("The DN '" + LogRedactionUtils.getSubjectDnLogSafe(principal.getName()) +
	...
	public static HashID getFromKeyID(X509Certificate cert) {
		...
		if ( id.isOK ) {
			if ( log.isDebugEnabled() ) {
				log.debug("The certificate with subject DN '" + LogRedactionUtils.getSubjectDnLogSafe(cert.getSubjectX500Principal().getName()) +
						"' can be fetched with 'search.cgi?sKIDHash=" + id.b64 + "' from the VA.");
			}
		} else {
			log.error("The certificate with subject DN '" + LogRedactionUtils.getSubjectDnLogSafe(cert.getSubjectX500Principal().getName()) +
