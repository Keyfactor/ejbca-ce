...
<body class="popup" id="editendentity"
      onload='<% if(usekeyrecovery) out.write(" isKeyRecoveryPossible(); ");%>
                 fillCAField();'>
  ...
  <% } else {
	...
	<form name="changeprofile" action="<%= THIS_FILENAME %>" method="post">
		...
		<tr id="Row<%=(row)%2%>">
		  ...
		  <td>
		  	<select name="<%=SELECT_ENDENTITYPROFILE %>" size="1" tabindex="<%=tabindex++%>" onchange="<% session.setAttribute("userData", userdata); %> document.changeprofile.submit()">
	...
	<td align="right"><%= ejbcawebbean.getText("PASSWORDORENROLLMENTCODE") %></td>
        <td>   
           ...
           <select name="<%= SELECT_PASSWORD %>" size="1" tabindex="<%=tabindex++%>">
               <% if (profile.getPredefinedPassword() != null){ %>
             <option value='<c:out value="<%= profile.getPredefinedPassword().trim() %>"/>' >
               <c:out value="<%= profile.getPredefinedPassword().trim() %>"/> 
             </option>
               <%   
                  }
                %>
	...
	<td>&nbsp;</td>
	 ...
	 <td>
         <select class="select-list" name="<%= SELECT_TOKEN %>" size="1" tabindex="<%=tabindex++%>" onchange='<% if(usekeyrecovery) out.write(" isKeyRecoveryPossible();");%>'>
         <%
    ...
    <% if (profile.isNameConstraintsExcludedUsed()) {
        ...
        <tr id="Row<%=(row++)%2%>">
            <td align="right">
                ...
                <%= ejbcawebbean.getHelpReference("/CA_Fields.html#Name_Constraints") %>
                <p class="help"><c:out value="<%= ejbcawebbean.getText(\"EXT_PKIX_NC_EXCLUDED_HELP1\") %>"/><br />
                <c:out value="<%= ejbcawebbean.getText(\"EXT_PKIX_NC_EXCLUDED_HELP2\") %>"/></p>
            </td>
            <td>
