	 ...
	 <td>      
          <% 
           ...
           <% }else{
             ...
             %> 
             <input type="text" name="<%= TEXTFIELD_SUBJECTDN + i %>" size="40" maxlength="255" tabindex="<%=tabindex++%>" value="<c:out value="<%= userdata.getSubjectDNField(DnComponents.profileIdToDnId(fielddata[EndEntityProfile.FIELDTYPE]),fielddata[EndEntityProfile.NUMBER]) %>"/>" <% if (regex != null) { %>pattern="<c:out value='<%=regex%>'/>"<% } %>>
           <% }
            }
            else{ %>
