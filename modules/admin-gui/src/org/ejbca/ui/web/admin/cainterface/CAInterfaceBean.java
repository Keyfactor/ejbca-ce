...
public class CAInterfaceBean implements Serializable {
    ...
    public String importCRL(final String caname, final byte[] crlBytes) {
        ...
        try {
            ...
            if(StringUtils.equals(cainfo.getSubjectDN(), CertTools.getIssuerDN(x509crl))) {
                retMsg = importCrl(cainfo, x509crl, false, false);
            } else {
                retMsg = "Error: The CRL in the file in not issued by " + caname;
            }
            
        } catch (CRLException e) {
            retMsg = "Error: Failed to read CRL from file. " + e.getLocalizedMessage();
        } catch (CADoesntExistsException e) {
    ...
    private String importCrl(final CAInfo cainfo, final X509CRL x509crl, final boolean strict, 
            final boolean adaptive) {

        final String MISSING_USERNAME_PREFIX = "*** Missing During CRL Import to: ";
        ...
        try {
            ...
            final String missing_user_name = MISSING_USERNAME_PREFIX + cainfo.getName();
            @SuppressWarnings("unchecked")
            ...
            if (entries != null) {
                for (final X509CRLEntry entry : entries) {
                    ...
                    final String username = certificatesession.findUsernameByCertSerno(serialNr, issuer);
                    ...
                    if (username == null) {
                        ...
                        final EndEntityInformation missingUserEndEntityInformation = ejbLocalHelper.getEndEntityAccessSession().findUser(authenticationToken, missing_user_name);
                        if (missingUserEndEntityInformation == null) {
                            ...
                            ejbLocalHelper.getEndEntityManagementSession().addUser(authenticationToken,
                                    userdataNew, false);
                            log.info("User '" + missing_user_name + "' has been added.");
                            ejbLocalHelper.getEndEntityManagementSession().setUserStatus(authenticationToken,
                                    missing_user_name, EndEntityConstants.STATUS_REVOKED);
                            log.info("User '" + missing_user_name + "' has been updated.");
                        }
                        certificatesession.storeCertificate(authenticationToken, certificate, missing_user_name, fingerprint, CertificateConstants.CERT_ACTIVE,
                    ...
                    if (!strict && certificatesession.isRevoked(issuer, serialNr)) {
                    ...
                    try {
                        ...
                        ejbLocalHelper.getEndEntityManagementSession().revokeCert(authenticationToken,
            ...
            if (crlStoreSession.getLastCRLNumber(issuer, false) < crl_no) {
                crlStoreSession.storeCRL(authenticationToken, x509crl.getEncoded(),
        ...
        return "CRL was imported successfully";
    }
