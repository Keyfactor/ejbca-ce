...
if ! [ -f $JBOSS_HOME/server/default/lib/jce-jdk13-117.jar ]
then
  cp lib/jce-jdk13-117.jar $JBOSS_HOME/server/default/lib
  echo Copied jce-jdk13-117.jar to $JBOSS_HOME/server/default/lib. JBoss must be restared.
fi
if ! [ -f $JBOSS_HOME/server/default/lib/bcmail-jdk13-117.jar ]
then
  cp lib/bcmail-jdk13-117.jar $JBOSS_HOME/server/default/lib
  echo Copied bcmail-jdk13-117.jar to $JBOSS_HOME/server/default/lib. JBoss must be restared.
fi
