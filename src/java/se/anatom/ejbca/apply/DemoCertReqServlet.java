 ...
 * </dt>
 * <dd>
 * The username (for EJBCA use only).  Optional, defaults to the CN in the PKCS#10 request.
 * </dd>
 * <dt>
 * password
 * </dt>
 ...
 * Used for authorization of the certificate request.
 * </dd>
 * <dt>
 * email
 * </dt>
 * <dd>
 * Email of the user for inclusion in subject alternative names.  Optional, defaults to none.
 * </dd>
 * <dt>
 * entityprofile
 * </dt>
 * <dd>
 * The name of the EJBCA end entity profile for the user.  Optional, defaults to an empty end
 * entity profile.
 * </dd>
 * <dt>
 * certificateprofile
 * </dt>
 * <dd>
 * The name of the EJBCA certificate profile to use.  Optional, defaults to the fixed end user
 * profile.
 * </dd>
 * </dl>
 * 
...
public class DemoCertReqServlet extends HttpServlet {
    ...
    private static final byte[] BEGIN_CERT = "-----BEGIN CERTIFICATE-----".getBytes();
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            useradminsessionhome = (IUserAdminSessionHome) javax.rmi.PortableRemoteObject.narrow(ctx.lookup(
                        "UserAdminSession"), IUserAdminSessionHome.class);
            raadminsessionhome = (IRaAdminSessionHome) javax.rmi.PortableRemoteObject.narrow(ctx.lookup(
                        "RaAdminSession"), IRaAdminSessionHome.class);
            storesessionhome = (ICertificateStoreSessionHome) javax.rmi.PortableRemoteObject.narrow(ctx.lookup(
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ((tmp = request.getParameter("entityprofile")) != null) {
            eProfileId = raadminsession.getEndEntityProfileId(admin,
        ...
        if ((tmp = request.getParameter("certificateprofile")) != null) {
            cProfileId = storesession.getCertificateProfileId(admin,
        ...
        try {
            useradminsession.addUser(admin, newuser.getUsername(), newuser.getPassword(),
                newuser.getSubjectDN(), newuser.getSubjectAltName(), newuser.getEmail(),
                newuser.getClearTextPassword(), newuser.getEndEntityProfileId(),
                newuser.getCertificateProfileId(), newuser.getType(), newuser.getTokenType(),
    ...
    private void sendNewCertToIEClient(byte[] b64cert, OutputStream out)
        ...
        BufferedReader br = new BufferedReader(new InputStreamReader(
                    getServletContext().getResourceAsStream(getInitParameter("responseTemplate"))));
    ...
    private void sendNewB64Cert(byte[] b64cert, HttpServletResponse out)
        ...
        out.setContentLength(b64cert.length + BEGIN_CERT_LENGTH + END_CERT_LENGTH +
            (3 * NL_LENGTH));
    ...
    /**
     * Verifies a username in the database
     *
     * @param admin administrator doing the request
     * @param username username to be verified
     * @param adminsession UserAdminSession to use for the query
     *
    ...
    private final boolean checkUsername(Admin admin, String username,
        IUserAdminSessionRemote adminsession) throws ServletException {
        if (username != null) {
