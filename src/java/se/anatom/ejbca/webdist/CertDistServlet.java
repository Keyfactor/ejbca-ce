 ...
 * </li>
 * <li>
 * lastcert - gets latest certificate of a user, takes argument 'subject=subjectDN'.
 * </li>
 * <li>
 * listcerts - lists all certificates of a user, takes argument 'subject=subjectDN'.
 * </li>
 * <li>
 * revoked - checks if a certificate is revoked, takes arguments 'subject=subjectDN&serno=serial
 * number'.
 * </li>
 * <li>
...
public class CertDistServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            storehome = (ICertificateStoreSessionHome) PortableRemoteObject.narrow(ctx.lookup(
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        if (command.equalsIgnoreCase(COMMAND_CRL)) {
            try {
                ...
                command.equalsIgnoreCase(COMMAND_LISTCERT)) {
            ...
            try {
                ...
                if (command.equalsIgnoreCase(COMMAND_CERT)) {
                    ...
                    if (latestcertno > -1) {
                        ...
                        log.debug("Sent latest certificate for '" + dn + "' to client at " +
                            remoteAddr);
                    } else {
                ...
                if (command.equalsIgnoreCase(COMMAND_LISTCERT)) {
                    ...
                    for (int i = 0; i < certs.length; i++) {
                        ...
                        pout.println("</pre>");
                        pout.println("<a href=\"certdist?cmd=revoked&issuer=" + issuer + "&serno=" +
                            serno.toString() + "\">Check if certificate is revoked</a>");
                        pout.println("<hr>");
                    }
                ...
                if (command.equalsIgnoreCase(COMMAND_NSCACERT)) {
                    ...
                    if (pkcs7) {
                        res.setHeader("Content-disposition",
                            "attachment; filename=" + filename + ".p7c");
                    } else {
                        res.setHeader("Content-disposition",
                            "attachment; filename=" + filename + ".crt");
                    }
            ...
            if (dn == null) {
                res.sendError(HttpServletResponse.SC_BAD_REQUEST,
                    "Usage command=revoked?issuer=<subjectdn>&serno=<serianlnumber>.");
            ...
            try {
                ...
                if (revinfo != null) {
                    ...
                    pout.println("Certificate with issuer '" + dn + "' and serial number '" +
                        serno + "' is revoked");
                    pout.println("RevocationDate is '" + revinfo.getRevocationDate() +
                        "' and reason '" + revinfo.getReason() + "'.");
                } else {
                    pout.println("<h1>NOT REVOKED</h1>");
                    pout.println("Certificate with issuer '" + dn + "' and serial number '" +
                        serno + "' is NOT revoked");
                }
