...
public class LocalAuthenticationSessionBean extends BaseSessionBean {
    ...
    public UserAuthData authenticateUser(Admin admin, String username, String password)
        ...
        try {
            ...
            if ((status == UserDataRemote.STATUS_NEW) || (status == UserDataRemote.STATUS_FAILED) ||
                    (status == UserDataRemote.STATUS_INPROCESS) ||
                    (status == UserDataRemote.STATUS_KEYRECOVERY)) {
                debug("Trying to authenticate user: username=" + data.getUsername() + ", dn=" +
                    data.getSubjectDN() + ", email=" + data.getSubjectEmail() + ", status=" +
                    data.getStatus() + ", type=" + data.getType());

                if (data.comparePassword(password) == false) {
                    try {
                        logsession.log(admin, LogEntry.MODULE_CA, new java.util.Date(), username,
                            null, LogEntry.EVENT_ERROR_USERAUTHENTICATION,
                            "Got request for user with invalid password.");
                ...
                try {
                    logsession.log(admin, LogEntry.MODULE_CA, new java.util.Date(), username, null,
                        LogEntry.EVENT_INFO_USERAUTHENTICATION, "Authenticated user.");
                } catch (RemoteException re) {
                    throw new EJBException(re);
                }

                UserAuthData ret = new UserAuthData(data.getUsername(), data.getSubjectDN(),
                        data.getSubjectAltName(), data.getSubjectEmail(), data.getType(),
                ...
                try {
                    logsession.log(admin, LogEntry.MODULE_CA, new java.util.Date(), username, null,
                        LogEntry.EVENT_ERROR_USERAUTHENTICATION,
                        "Got request with status '" + status +
                        "', NEW, FAILED or INPROCESS required.");
                } catch (RemoteException re) {
                    throw new EJBException(re);
                }

                throw new AuthStatusException("User " + username + " has status '" + status +
                    "', NEW, FAILED or INPROCESS required.");
            }
        } catch (ObjectNotFoundException oe) {
            try {
                logsession.log(admin, LogEntry.MODULE_CA, new java.util.Date(), username, null,
                    LogEntry.EVENT_ERROR_USERAUTHENTICATION, "Got request for nonexisting user.");
            } catch (RemoteException re) {
                throw new EJBException(re);
            }
    ...
    public void finishUser(Admin admin, String username, String password)
        ...
        try {
            ...
            logsession.log(admin, LogEntry.MODULE_CA, new java.util.Date(), username, null,
                LogEntry.EVENT_INFO_CHANGEDENDENTITY, "Changed status to STATUS_GENERATED.");
            debug("<finishUser(" + username + ", hiddenpwd)");
        } catch (ObjectNotFoundException oe) {
            try {
                logsession.log(admin, LogEntry.MODULE_CA, new java.util.Date(), username, null,
