...
public class LocalEjbcaHardTokenBatchJobSessionBean extends BaseSessionBean {
    ...
    private IHardTokenSessionLocal getHardTokenSession() {
        if (hardtokensession == null) {
            try {
                IHardTokenSessionLocalHome hardtokensessionhome = (IHardTokenSessionLocalHome) lookup("java:comp/env/ejb/HardTokenSession",
    ...
    public UserAdminData getNextHardTokenToGenerate(Admin admin, X509Certificate issuercert)
        ...
        if (issuerid != IHardTokenSessionLocal.NO_ISSUER) {
            ...
            try {
                // Construct SQL query.
                con = getConnection();
                ps = con.prepareStatement("select " + USERDATA_COL +
                        " from UserData where hardTokenIssuerId = " + issuerid +
                        " and tokenType > " + SecConst.TOKEN_SOFT);
                ...
                // Assemble result.
                if (rs.next()) {
                    returnval = new UserAdminData(rs.getString(1), rs.getString(2),
                ...
                if (returnval != null) {
                    ...
                    getLogSession().log(admin, LogEntry.MODULE_HARDTOKEN, new java.util.Date(),
                        returnval.getUsername(), null, LogEntry.EVENT_ERROR_HARDTOKEN_USERDATASENT,
                        "Userdata sent for token generation to issuer with dn: " +
                        CertTools.getSubjectDN(issuercert));
                }
            } catch (Exception e) {
                try {
                    getLogSession().log(admin, LogEntry.MODULE_HARDTOKEN, new java.util.Date(),
                        null, null, LogEntry.EVENT_ERROR_HARDTOKEN_USERDATASENT,
                        "Error when retrieving next token for issuer with dn: " +
                        CertTools.getSubjectDN(issuercert));
     ...
     * of MAX_RETURNED_QUEUE_SIZE users will be returned by call.
     *
     * @param admin the administrator performing the actions
     * @param issuercert the certificate of the hard token issuer.
     *
    ...
    public Collection getNextHardTokensToGenerate(Admin admin, X509Certificate issuercert)
        ...
        if (issuerid != IHardTokenSessionLocal.NO_ISSUER) {
            ...
            try {
                ...
                // Assemble result.
                while (rs.next() &&
                        (returnval.size() <= IHardTokenBatchJobSessionLocal.MAX_RETURNED_QUEUE_SIZE)) {
                    ...
                    returnval.add(data);
                    getLogSession().log(admin, LogEntry.MODULE_HARDTOKEN, new java.util.Date(),
                        data.getUsername(), null, LogEntry.EVENT_ERROR_HARDTOKEN_USERDATASENT,
                        "Userdata sent for token generation to issuer with dn: " +
                        CertTools.getSubjectDN(issuercert));
                }
            } catch (Exception e) {
                try {
                    getLogSession().log(admin, LogEntry.MODULE_HARDTOKEN, new java.util.Date(),
                        null, null, LogEntry.EVENT_ERROR_HARDTOKEN_USERDATASENT,
                        "Error when retrieving next tokens for issuer with dn: " +
                        CertTools.getSubjectDN(issuercert));
                } catch (RemoteException re) {
                }
        ...
        if (issuerid != IHardTokenSessionLocal.NO_ISSUER) {
            ...
            try {
                ...
                ps = con.prepareStatement(
                        "select COUNT(*) from UserData where hardTokenIssuerId = " + issuerid +
                        " and tokenType > " + SecConst.TOKEN_SOFT);
     ...
     * This function is mainly for avoiding desyncronisation when a hard token issuer is deleted.
     *
