...
public class CaInitCommand extends BaseCaAdminCommand {
    ...
    public void execute() throws IllegalAdminCommandException, ErrorAdminCommandException {
        try {
            ...
            ISignSessionHome signhome = (ISignSessionHome) javax.rmi.PortableRemoteObject.narrow(context.lookup(
            ...
            for (int j = 0; j < certs.length; j++) {
                ...
                try {
                    ...
                    certificateStore.storeCertificate(administrator, cert, "cainit", cafingerprint,
                        CertificateData.CERT_ACTIVE, type);
                } catch (java.rmi.ServerException e) {
                    System.out.println("Certificate for subject '" + CertTools.getSubjectDN(cert) +
                        "' already exist in the certificate store.");
                }

                for (int i = 0; i < publishers.size(); i++) {
                    boolean ret = ((IPublisherSessionRemote) (publishers.get(i))).storeCertificate(administrator,
                            cert, "cainit", cafingerprint, CertificateData.CERT_ACTIVE, type);

                    if (ret == true) {
                        System.out.println("Published certificate in publisher no " + i + 1);
                    } else {
                        System.out.println("Failed to publish certificate in publisher no " + i +
                            1);
                    }
                }
        ...
        if (publishers == null) {
            ...
            try {
                while (true) {
                    ...
                    IPublisherSessionHome pubhome = (IPublisherSessionHome) javax.rmi.PortableRemoteObject.narrow(context.lookup(
                                jndiName), IPublisherSessionHome.class);
                    IPublisherSessionRemote pub = pubhome.create();
                    publishers.add(pub);
                    debug("Added publisher class '" + pub.getClass().getName() + "' at position " +
                        i);
                    i++;
                }
            } catch (NamingException e) {
