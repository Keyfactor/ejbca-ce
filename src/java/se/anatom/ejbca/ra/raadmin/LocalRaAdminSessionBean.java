 ...
 *
 * @version $Id: LocalRaAdminSessionBean.java,v 1.4 2002-06-13 13:52:57 anatom Exp $
 */
public class LocalRaAdminSessionBean extends BaseSessionBean  {
    ...
    public GlobalConfiguration loadGlobalConfiguration()  {
        ...
        try {
            ...
            debug("Loaded Global Configuration " + gcdata.getGlobalConfiguration().getEjbcaTitle());
    ...
    public void saveGlobalConfiguration(GlobalConfiguration globalconfiguration)  {
        ...
        try {
           ...
           GlobalWebConfigurationDataLocal gcdata = globalconfigurationhome.findByPrimaryKey(pk);
           if (gcdata == null) {
               error("findByPrimaryKey() returnerade null");
               throw new javax.ejb.FinderException();
           }
           GlobalConfiguration gc = gcdata.getGlobalConfiguration();
           debug("Old Global Configuration " + gc.getEjbcaTitle());
           debug("Setting new GlobalConfiguration "+ globalconfiguration.getEjbcaTitle());
           gcdata.setGlobalConfiguration(globalconfiguration);
           //globalconfigurationhome.remove(pk);

        } catch (javax.ejb.FinderException fe) {
            info("Cannot find GlobalConfiguration");
            try {
                GlobalWebConfigurationDataLocal data1= globalconfigurationhome.create(pk,globalconfiguration);
            } catch (CreateException ce) {
                error("Cannot create new GlobalConfiguration!");
                throw new EJBException(ce);
            }
        } catch (Exception e){
             error("Error saving GlobalConfiguration: ", e);
             throw new EJBException(e);
        }
        debug("<saveGlobalConfiguration()");
