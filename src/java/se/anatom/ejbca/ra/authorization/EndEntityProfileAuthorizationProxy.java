...
/**
 * A class used to improve performance by proxying a users end entity profile authorization
 * minimizing the need of traversing trough the authorization tree and rmi lookups. It's use
 * should only be within short time to avoid desyncronisation.
 *
 * @author TomSelleck
 */
public class EndEntityProfileAuthorizationProxy implements Serializable {
     ...
     *
     * @param authorizationsession DOCUMENT ME!
     */
    ...
    /**
     * Creates a new EndEntityProfileAuthorizationProxy object.
     *
     * @param authorizationsession DOCUMENT ME!
     */
    ...
    /**
     * Method that first tries to authorize a users profile right in local hashmap and if it
     * doesn't exists looks it up over RMI.
     *
     ...
     */
    public boolean getEndEntityProfileAuthorization(Admin admin, int profileid, String rights,
        int module) throws RemoteException {
        return isAuthorized(admin, profileid, rights, true, module);
    }

    /**
     * Method that first tries to authorize a users profile right in local hashmap and if it
     * doesn't exists looks it up over RMI, without performing any logging.
     *
     * @param admin DOCUMENT ME!
     * @param profileid the profile to look up.
     * @param rights which profile rights to look for.
     ...
     * @return the profilename or null if no profilename is relatied to the given id
     */
    public boolean getEndEntityProfileAuthorizationNoLog(Admin admin, int profileid, String rights)
        throws RemoteException {
        return isAuthorized(admin, profileid, rights, false, 0);
    }

    // Private Methods
    public boolean isAuthorized(Admin admin, int profileid, String rights, boolean log, int module)
        throws RemoteException {
        ...
        resource = adm + GlobalConfiguration.ENDENTITYPROFILEPREFIX + Integer.toString(profileid) +
            rights;
        ...
        if ((returnval != null) && log) {
            if (returnval.booleanValue()) {
                getLogSessionBean().log(admin, module, new java.util.Date(), null, null,
                    LogEntry.EVENT_INFO_AUTHORIZEDTORESOURCE,
                    "Resource : " + GlobalConfiguration.ENDENTITYPROFILEPREFIX +
                    Integer.toString(profileid) + rights);
            } else {
                getLogSessionBean().log(admin, module, new java.util.Date(), null, null,
                    LogEntry.EVENT_ERROR_NOTAUTHORIZEDTORESOURCE,
                    "Resource : " + GlobalConfiguration.ENDENTITYPROFILEPREFIX +
                    Integer.toString(profileid) + rights);
            }
        }
        ...
        if (returnval == null) {
            ...
            try {
                if (local) {
                    if (log) {
                        authorizationsessionlocal.isAuthorized(admin.getAdminInformation(),
                            GlobalConfiguration.ENDENTITYPROFILEPREFIX +
                            Integer.toString(profileid) + rights);
                    } else {
                        authorizationsessionlocal.isAuthorizedNoLog(admin.getAdminInformation(),
                            GlobalConfiguration.ENDENTITYPROFILEPREFIX +
                            Integer.toString(profileid) + rights);
                    }
                } else {
                    if (log) {
                        authorizationsessionremote.isAuthorized(admin.getAdminInformation(),
                            GlobalConfiguration.ENDENTITYPROFILEPREFIX +
                            Integer.toString(profileid) + rights);
                    } else {
                        authorizationsessionremote.isAuthorizedNoLog(admin.getAdminInformation(),
                            GlobalConfiguration.ENDENTITYPROFILEPREFIX +
                            Integer.toString(profileid) + rights);
                    }
                }
