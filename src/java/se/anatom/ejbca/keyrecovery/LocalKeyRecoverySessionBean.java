...
public class LocalKeyRecoverySessionBean extends BaseSessionBean  {
    ...
    public boolean addKeyRecoveryData(Admin admin, X509Certificate certificate, String username, KeyPair keypair){
       ...
       boolean returnval = false;
        try {
            // TODO, encrypt keys.
            keyrecoverydatahome.create(certificate.getSerialNumber(), CertTools.getIssuerDN(certificate), username, keypair);
            getLogSession().log(admin, LogEntry.MODULE_KEYRECOVERY, new java.util.Date(),username, certificate, LogEntry.EVENT_INFO_KEYRECOVERY,"Keyrecovery data for certificate with serial number : " + certificate.getSerialNumber().toString(16) +  ", " + CertTools.getIssuerDN(certificate) + " added.");
            returnval=true;
        }
        catch (Exception e) {
          try{
            getLogSession().log(admin, LogEntry.MODULE_KEYRECOVERY, new java.util.Date(),username, certificate, LogEntry.EVENT_ERROR_KEYRECOVERY,"Error when trying to add keyrecovery data for certificate with serial number : " + certificate.getSerialNumber().toString(16) +  ", " + CertTools.getIssuerDN(certificate) + ".");
    ...
    public boolean changeKeyRecoveryData(Admin admin,  X509Certificate certificate, boolean markedasrecoverable, KeyPair keypair) {
       debug(">changeKeyRecoveryData(certsn: " +  certificate.getSerialNumber().toString() + ", " + CertTools.getIssuerDN(certificate) + ")");
       boolean returnval = false;
       try {
            KeyRecoveryDataLocal krd = keyrecoverydatahome.findByPrimaryKey(new KeyRecoveryDataPK(certificate.getSerialNumber(),CertTools.getIssuerDN(certificate)));
            krd.setMarkedAsRecoverable(markedasrecoverable);
            // TODO, encrypt keys.
            krd.setKeyPair(keypair);
            getLogSession().log(admin, LogEntry.MODULE_KEYRECOVERY, new java.util.Date(),krd.getUsername(), certificate, LogEntry.EVENT_INFO_KEYRECOVERY,"Keyrecovery data for certificate with serial number : " + certificate.getSerialNumber().toString(16) + ", " + CertTools.getIssuerDN(certificate) + " changed.");
            returnval=true;
        }
        catch (Exception e) {
          try{
            getLogSession().log(admin, LogEntry.MODULE_KEYRECOVERY, new java.util.Date(),null, certificate, LogEntry.EVENT_ERROR_KEYRECOVERY,"Error when trying to update keyrecovery data for certificate with serial number : " + certificate.getSerialNumber().toString(16) + ", " + CertTools.getIssuerDN(certificate) + ".");
          }catch(RemoteException re){
             throw new EJBException(e);
          }
        }
    ...
    public void removeKeyRecoveryData(Admin admin, X509Certificate certificate) {
      ...
      try{
        ...
        krd.remove();
        getLogSession().log(admin, LogEntry.MODULE_KEYRECOVERY, new java.util.Date(),username, certificate, LogEntry.EVENT_INFO_KEYRECOVERY,"Keyrecovery data for certificate with serial number : " + certificate.getSerialNumber().toString(16) + ", " + CertTools.getIssuerDN(certificate) + " removed.");
      }catch(Exception e){
         try{
           getLogSession().log(admin, LogEntry.MODULE_KEYRECOVERY, new java.util.Date(),null, certificate, LogEntry.EVENT_ERROR_KEYRECOVERY,"Error when removing keyrecovery data for certificate with serial number : " + certificate.getSerialNumber().toString(16) + ", " + CertTools.getIssuerDN(certificate) + ".");
         }catch(Exception re){
            throw new EJBException(e);
         }
      }
