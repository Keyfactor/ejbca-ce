...
public abstract class CA extends UpgradeableDataHashMap implements Serializable {
	...
	private void createExtendedCAService(ExtendedCAServiceInfo info) {
		// Create implementation using reflection
		   try {
			   ...
			   final ExtendedCAService service = (ExtendedCAService)implClass.getConstructor(ExtendedCAServiceInfo.class).newInstance(new Object[] {info});
     ...
     * @throws IllegalCryptoTokenException If the token keystore is invalid (crypto error thrown by crypto provider), or the CA token type is undefined.
     * @throws  
     */
     ...
     * @throws Exception
     */
    public abstract Certificate generateCertificate(EndEntityInformation subject, 
     ...
     */
    public abstract byte[] createRequest(Collection<DEREncodable> attributes, String signAlg, Certificate cacert, int signatureKeyPurpose) throws CryptoTokenOfflineException;
