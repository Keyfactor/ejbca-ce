...
public class CAAdminSessionBean implements CAAdminSessionLocal, CAAdminSessionRemote {
    ...
    private void createCRLs(Admin admin, CA ca, CAInfo cainfo) throws CATokenOfflineException {
        final String fp = this.crlStoreSession.run(admin, ca);
        ...
        if (fp != null) {
            ...
            if (cainfo.getDeltaCRLPeriod() > 0) {
                this.crlStoreSession.runDeltaCRL(admin, ca, crlInfo.getLastCRLNumber(), crlInfo.getCreateDate().getTime());
            }
        }
    }
    ...
    public int createCRLs(Admin admin, Collection<Integer> caids, long addtocrloverlaptime) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (crlStoreSession.runNewTransactionConditioned(admin, ca, addtocrloverlaptime)) {
                    createdcrls++;
                }
            }
        } catch (Exception e) {
    ...
    public int createDeltaCRLs(Admin admin, Collection<Integer> caids, long crloverlaptime) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                CA ca = getCA(admin, caid);
                if (crlStoreSession.runDeltaCRLnewTransactionConditioned(admin, ca, crloverlaptime)) {
                    createddeltacrls++;
                }
            }
        } catch (Exception e) {
