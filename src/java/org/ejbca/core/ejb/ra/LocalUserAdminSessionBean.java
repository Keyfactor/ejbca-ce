...
public class LocalUserAdminSessionBean extends BaseSessionBean {
    ...
    public void addUser(Admin admin, UserDataVO userdata, boolean clearpwd) throws AuthorizationDeniedException, UserDoesntFullfillEndEntityProfile, DuplicateKeyException, ApprovalException, WaitingForApprovalException {
        ...
        try {
            ...
            if (profile.useAutoGeneratedPasswd() && !clearpwd) {
                userdata.setPassword(newpassword);	// We want to be able to process the generated password in the notification ...
            }
            // Send notifications, if they should be sent
            sendNotification(admin, userdata, UserDataConstants.STATUS_NEW);
            if (profile.useAutoGeneratedPasswd() && !clearpwd) {
                userdata.setPassword(null);	// ... but nowhere else
            }
...
throws AuthorizationDeniedException, UserDoesntFullfillEndEntityProfile, ApprovalException, WaitingForApprovalException {
    ...
    public void changeUser(Admin admin, UserDataVO userdata, boolean clearpwd)
        ...
        try {
            ...
            if (profile.useAutoGeneratedPasswd() && !clearpwd) {
                udata.setPassword(newpassword);	// We want to be able to process the generated password in the notification ...
            }
            // Send notification if it should be sent. 
            sendNotification(admin, udata, userdata.getStatus());
            if (profile.useAutoGeneratedPasswd() && !clearpwd) {
                udata.setPassword(null);	// ... but nowhere else
            }
