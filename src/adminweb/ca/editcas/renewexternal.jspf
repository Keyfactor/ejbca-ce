...
<div align="center">   
   <h2><%= ejbcawebbean.getText("RENEWCA") %><br></h2>
   <h3><%= ejbcawebbean.getText("CANAME")+ " : " + caname %> </h3>
   <h3><br><%= ejbcawebbean.getText("MAKEREQUEST") %></h3>
</div>
  <table width="100%" border="0" cellspacing="3" cellpadding="3">
   ...
   <!--     <A  onclick='displayHelpWindow("<%= ejbcawebbean.getHelpfileInfix("ca_help.html") + "#cas"%>")'>
    ...
    <tr  id="Row<%=row++%2%>"> 
      <td width="49%" valign="top" align="right"><%= ejbcawebbean.getText("MAKEREQUESTHELP") %></td>
      <td width="51%" valign="top">     
        <input TYPE="FILE" NAME="<%= FILE_RECIEVEFILE %>">            
        <input type="submit" name="<%= BUTTON_RECIEVEFILE %>"  value="<%= ejbcawebbean.getText("MAKEREQUEST") %>" ><br><br>        
      </td>
    </tr>
    </form>
  </table>
 ...
 <table width="100%" border="0" cellspacing="3" cellpadding="3">
    <form name="recieveresponse" action="<%= THIS_FILENAME %>" method="post" enctype='multipart/form-data' onSubmit='return check(document.recieveresponse.<%= FILE_RECIEVEFILE %>)'>
      ...
      <input type="hidden" name='<%= HIDDEN_CANAME %>' value='<%= caname%>'>
    <tr  id="Row<%=row++%2%>"> 
      <td width="49%" valign="top" align="right"><%= ejbcawebbean.getText("RECIEVEREQUESTHELP") %></td>
      <td width="51%" valign="top">     
        <input TYPE="FILE" NAME="<%= FILE_RECIEVEFILE %>">            
        <input type="submit" name="<%= BUTTON_RECIEVEFILE %>"  value="<%= ejbcawebbean.getText("RECIEVEREQUEST") %>" ><br><br>
